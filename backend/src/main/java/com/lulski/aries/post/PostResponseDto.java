...
public record PostResponseDto(List<PostDto> postDto, String message, int page, int size, long total) {
    public static DateTimeFormatter FORMATTER = DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM).withLocale(Locale.ENGLISH);
    ...
    public static PostDto fromPost(Post post) {
        ...
        return new PostDto(Objects.toString(post.getTitle(), "-")
            ...
            , Objects.toString(post.getAuthor(), "-")
            , Objects.toString(post.getId().toString(), "-")
    ...
    public static List<PostDto> fromPosts(List<Post> posts) {
        if (posts==null) throw new IllegalArgumentException("posts must not be null");
